...
function updateDiagnostics(document, collection) {
    ...
    lines.forEach((line, lineIndex) => {
        ...
        if (trimmedLine.match(/^\s*(var|const|let|type)\s+[a-zA-Z_][a-zA-Z0-9_]*\s*$/) && !trimmedLine.includes('=')) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(lineIndex, 0, lineIndex, line.length), 'Variable declaration should include initialization', vscode.DiagnosticSeverity.Warning);
            diagnostics.push(diagnostic);
        }
        ...
        if (trimmedLine.includes('Time.now()') && !text.includes('import std.time')) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(lineIndex, 0, lineIndex, line.length), 'Missing import: add "import std.time" at the top of the file', vscode.DiagnosticSeverity.Warning);
            diagnostics.push(diagnostic);
        }
    });
